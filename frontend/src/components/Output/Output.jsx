...
class Output extends React.Component {
  ...
  render() {
    ...
    if (this.state.loading) {
      ...
      return (
        <div className="output-wrapper">
          {manualLabelText}
          <button onClick={this.removeAnnotations}>Remove annotations</button>
  ...
  onSubmit = () => {
    const boxes = this.repackMarks(this.state.originalWidth, this.state.originalHeight);
    this.setState({ loading: true })
    // PUT request to box update API
    axios.put(`box/update?image_id=${this.state.imageHash}`, boxes)
      .then(() => {
        this.setState({ loading: false })
        alert("Successfully updated box annotations in database!")
      })
      .catch(err => {
        this.setState({ loading: false })
        alert(err)
  ...
  getRatio = (incomingWidth, incomingHeight) => {
    ...
    if (incomingWidth < resizedWindowWidth && incomingHeight < resizedWindowHeight) {
      return 1
    } else {
  ...
  unpackMarks = (rawJsonArray, originalWidth, originalHeight) => {
    let newArray = []
    rawJsonArray.forEach(element => {
      ...
      newArray.push({
        ...
        mark: {
          type: "RECT",
          x: element.x * originalWidth,
          y: element.y * originalHeight,
          width: element.width * originalWidth,
          height: element.height * originalHeight
        }
      })
    });
    return newArray;
  ...
  repackMarks = (originalWidth, originalHeight) => {
    let newArray = []
    this.state.annotations.forEach(element => {
      ...
      newArray.push({
        id: element.id,
        label: label,
        x: element.mark.x / originalWidth,
        y: element.mark.y / originalHeight,
        width: element.mark.width / originalWidth,
        height: element.mark.height / originalHeight,
        probability: 1
      })
    })
    return newArray;
  ...
  fetchImageBoxes = () => {
    ...
    axios.get(`image/get?id=${this.state.imageHash}`)
      .then((response) => {
        ...
        const boxes = this.unpackMarks(data.boxes, data.width, data.height)
        this.setState({
          ratio: newRatio,
          loading: false,
